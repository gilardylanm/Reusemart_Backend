...
<body>
    ...
    <header>
        ...
        <nav>
            ...
            <a href="{{ route('barang.perpanjangan') }}" class="active">Perpanjangan Penitipan Lanjutan</a>
                ...
                <table id="historyTable" class="table">
                    ...
                    <tbody>
                        ...
                        @foreach($barangList as $barang)
                            ...
                            @php
                                // Total potongan = 5% x total harga semua barang pada penitipan ini
                                $barangPenitipan = $barangList->where('ID_PENITIPAN', $barang->ID_PENITIPAN);
                                $totalHarga = $barangPenitipan->sum('HARGA_BARANG');
                            ...
                            <tr>
                                <!-- ID Barang -->
                                <td>
                                    <div class="item-name">{{ $barang->ID_BARANG }}</div>
                                </td>
                                ...
                                <td>
                                    <div class="item-name">
                                        {{ $barang->penitipan->STATUS_PERPANJANGAN ? 'Aktif' : 'Tidak Aktif' }}
                                    </div>
                                </td>
                                ...
                                @if($isFirstOfPenitipan)
                                    <td>
                                        @if ($barang->penitipan->ID_PENITIP == $penitipId)
                                            <!-- Tombol perpanjang hanya jika penitip yang login -->
                                            <form id="form-perpanjang-{{ $barang->penitipan->ID_PENITIPAN }}"
                                                action="{{ route('penitipan.perpanjang', $barang->penitipan->ID_PENITIPAN) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <button type="button" class="btn btn-sm btn-warning"
                                                    onclick="konfirmasiPerpanjang({{ $barang->penitipan->ID_PENITIPAN }}, {{ $totalHarga }}, {{ $potongan }})">
                                                    Perpanjang
                                                </button>
                                            </form>
                                        @endif
                            ...
                            @if($isFirstOfPenitipan)
                                @php $penitipanRendered[] = $barang->ID_PENITIPAN; @endphp
                            @endif
                        @endforeach
                    </tbody>
    ...
    <script>
        ...
        function konfirmasiPerpanjang(id, totalHarga, potongan) {
            const konfirmasi = confirm(
                `Apakah Anda yakin akan memperpanjang masa penitipan barang ini dengan harga ${formatRupiah(totalHarga)} dan pemotongan saldo sebesar ${formatRupiah(potongan)}?`
            );
